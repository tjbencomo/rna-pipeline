...
def createParser():
    ...
    parser.add_argument('-I', '--input', type=str, nargs='*', 
                        help='Specify input directory or input files.',
                        dest='input')
    ...
    parser.add_argument('-rDir', '--rsemDirectory', type=str, nargs=1,
                        help='Optional. Location of reference directory for RSEM. If not used, uses directory from rna-pipeline-config.ini',
                        dest='rsemDirectory')
    parser.add_argument('-countRef', '--htseqCountReferenceFile', type=str, nargs=1,
                        help='Optional. GTF file path for HTSEQ-Count. If not used, uses directory from rna-pipeline-config.ini',
...
def parseArgs():
    ...
    if len(args.input) == 1:
        ...
        if os.path.isdir(pipelineInput) is True:
...
def loadConfigData():
    ...
    config.read(os.path.join(userDirectory, 'rna-pipeline-config.ini'))
...
def getSamplePairs(directory):
    ...
    for f in files:
        ...
        print(match)
        samples.append((os.path.join(directory, f), os.path.join(directory, match)))
        files.remove(match)
